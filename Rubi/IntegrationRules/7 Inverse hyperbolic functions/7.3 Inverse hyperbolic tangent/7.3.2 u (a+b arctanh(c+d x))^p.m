...
Int[((a_.) + (b_.)*ArcTanh[(c_) + (d_.)*(x_)])^(p_), x_Symbol] := Unintegrable[(a + b*ArcTanh[c + d*x])^p, x] /; FreeQ[{a, b, c, d, p}, x] &&  !IGtQ[p, 0]
Int[((a_.) + (b_.)*ArcCoth[(c_) + (d_.)*(x_)])^(p_), x_Symbol] := Unintegrable[(a + b*ArcCoth[c + d*x])^p, x] /; FreeQ[{a, b, c, d, p}, x] &&  !IGtQ[p, 0]
...
Int[((e_.) + (f_.)*(x_))^(m_.)*((a_.) + (b_.)*ArcTanh[(c_) + (d_.)*(x_)])^(p_), x_Symbol] := Unintegrable[(e + f*x)^m*(a + b*ArcTanh[c + d*x])^p, x] /; FreeQ[{a, b, c, d, e, f, m, p}, x] &&  !IGtQ[p, 0]
Int[((e_.) + (f_.)*(x_))^(m_.)*((a_.) + (b_.)*ArcCoth[(c_) + (d_.)*(x_)])^(p_), x_Symbol] := Unintegrable[(e + f*x)^m*(a + b*ArcCoth[c + d*x])^p, x] /; FreeQ[{a, b, c, d, e, f, m, p}, x] &&  !IGtQ[p, 0]
...
Int[ArcTanh[(c_) + (d_.)*(x_)]/((e_) + (f_.)*(x_)^(n_)), x_Symbol] := Unintegrable[ArcTanh[c + d*x]/(e + f*x^n), x] /; FreeQ[{c, d, e, f, n}, x] &&  !RationalQ[n]
Int[ArcCoth[(c_) + (d_.)*(x_)]/((e_) + (f_.)*(x_)^(n_)), x_Symbol] := Unintegrable[ArcCoth[c + d*x]/(e + f*x^n), x] /; FreeQ[{c, d, e, f, n}, x] &&  !RationalQ[n]

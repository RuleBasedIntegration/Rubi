...
Int[((a_.) + (b_.)*ArcSech[(c_) + (d_.)*(x_)])^(p_), x_Symbol] := Unintegrable[(a + b*ArcSech[c + d*x])^p, x] /; FreeQ[{a, b, c, d, p}, x] &&  !IGtQ[p, 0]
Int[((a_.) + (b_.)*ArcCsch[(c_) + (d_.)*(x_)])^(p_), x_Symbol] := Unintegrable[(a + b*ArcCsch[c + d*x])^p, x] /; FreeQ[{a, b, c, d, p}, x] &&  !IGtQ[p, 0]
...
Int[((e_.) + (f_.)*(x_))^(m_.)*((a_.) + (b_.)*ArcSech[(c_) + (d_.)*(x_)])^(p_.), x_Symbol] := (-(d^(m + 1))^(-1))*Subst[Int[(a + b*x)^p*Sech[x]*Tanh[x]*(d*e - c*f + f*Sech[x])^m, x], x, ArcSech[c + d*x]] /; FreeQ[{a, b, c, d, e, f}, x] && IGtQ[p, 0] && IntegerQ[m]
Int[((e_.) + (f_.)*(x_))^(m_.)*((a_.) + (b_.)*ArcCsch[(c_) + (d_.)*(x_)])^(p_.), x_Symbol] := (-(d^(m + 1))^(-1))*Subst[Int[(a + b*x)^p*Csch[x]*Coth[x]*(d*e - c*f + f*Csch[x])^m, x], x, ArcCsch[c + d*x]] /; FreeQ[{a, b, c, d, e, f}, x] && IGtQ[p, 0] && IntegerQ[m]
...
Int[((e_.) + (f_.)*(x_))^(m_.)*((a_.) + (b_.)*ArcSech[(c_) + (d_.)*(x_)])^(p_), x_Symbol] := Unintegrable[(e + f*x)^m*(a + b*ArcSech[c + d*x])^p, x] /; FreeQ[{a, b, c, d, e, f, m, p}, x] &&  !IGtQ[p, 0]
Int[((e_.) + (f_.)*(x_))^(m_.)*((a_.) + (b_.)*ArcCsch[(c_) + (d_.)*(x_)])^(p_), x_Symbol] := Unintegrable[(e + f*x)^m*(a + b*ArcCsch[c + d*x])^p, x] /; FreeQ[{a, b, c, d, e, f, m, p}, x] &&  !IGtQ[p, 0]
...
Int[E^ArcSech[(a_.)*(x_)^(p_.)]/(x_), x_Symbol] := -(a*p*x^p)^(-1) + (Sqrt[1 + a*x^p]/a)*Sqrt[1/(1 + a*x^p)]*Int[Sqrt[1 + a*x^p]*(Sqrt[1 - a*x^p]/x^(p + 1)), x] /; FreeQ[{a, p}, x]
...
Int[ArcSech[u_], x_Symbol] := x*ArcSech[u] + (Sqrt[1 - u^2]/(u*Sqrt[-1 + 1/u]*Sqrt[1 + 1/u]))*Int[SimplifyIntegrand[x*(D[u, x]/(u*Sqrt[1 - u^2])), x], x] /; InverseFunctionFreeQ[u, x] &&  !FunctionOfExponentialQ[u, x]
Int[ArcCsch[u_], x_Symbol] := x*ArcCsch[u] - (u/Sqrt[-u^2])*Int[SimplifyIntegrand[x*(D[u, x]/(u*Sqrt[-1 - u^2])), x], x] /; InverseFunctionFreeQ[u, x] &&  !FunctionOfExponentialQ[u, x]
Int[((c_.) + (d_.)*(x_))^(m_.)*((a_.) + (b_.)*ArcSech[u_]), x_Symbol] := (c + d*x)^(m + 1)*((a + b*ArcSech[u])/(d*(m + 1))) + b*(Sqrt[1 - u^2]/(d*(m + 1)*u*Sqrt[-1 + 1/u]*Sqrt[1 + 1/u]))*Int[SimplifyIntegrand[(c + d*x)^(m + 1)*(D[u, x]/(u*Sqrt[1 - u^2])), x], x] /; FreeQ[{a, b, c, d, m}, x] && NeQ[m, -1] && InverseFunctionFreeQ[u, x] &&  !FunctionOfQ[(c + d*x)^(m + 1), u, x] &&  !FunctionOfExponentialQ[u, x]
Int[((c_.) + (d_.)*(x_))^(m_.)*((a_.) + (b_.)*ArcCsch[u_]), x_Symbol] := (c + d*x)^(m + 1)*((a + b*ArcCsch[u])/(d*(m + 1))) - b*(u/(d*(m + 1)*Sqrt[-u^2]))*Int[SimplifyIntegrand[(c + d*x)^(m + 1)*(D[u, x]/(u*Sqrt[-1 - u^2])), x], x] /; FreeQ[{a, b, c, d, m}, x] && NeQ[m, -1] && InverseFunctionFreeQ[u, x] &&  !FunctionOfQ[(c + d*x)^(m + 1), u, x] &&  !FunctionOfExponentialQ[u, x]
Int[(v_)*((a_.) + (b_.)*ArcSech[u_]), x_Symbol] := With[{w = IntHide[v, x]}, Dist[a + b*ArcSech[u], w, x] + b*(Sqrt[1 - u^2]/(u*Sqrt[-1 + 1/u]*Sqrt[1 + 1/u]))*Int[SimplifyIntegrand[w*(D[u, x]/(u*Sqrt[1 - u^2])), x], x] /; InverseFunctionFreeQ[w, x]] /; FreeQ[{a, b}, x] && InverseFunctionFreeQ[u, x] &&  !MatchQ[v, ((c_.) + (d_.)*x)^(m_.) /; FreeQ[{c, d, m}, x]]
Int[(v_)*((a_.) + (b_.)*ArcCsch[u_]), x_Symbol] := With[{w = IntHide[v, x]}, Dist[a + b*ArcCsch[u], w, x] - b*(u/Sqrt[-u^2])*Int[SimplifyIntegrand[w*(D[u, x]/(u*Sqrt[-1 - u^2])), x], x] /; InverseFunctionFreeQ[w, x]] /; FreeQ[{a, b}, x] && InverseFunctionFreeQ[u, x] &&  !MatchQ[v, ((c_.) + (d_.)*x)^(m_.) /; FreeQ[{c, d, m}, x]]
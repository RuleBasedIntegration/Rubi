...
(* ::Code:: *)
(* Int[(a_.+b_.*x_)^m_*(c_+d_.*x_)^m_,x_Symbol] :=
  (a+b*x)^m*(c+d*x)^m/((a+b*x)*(c+d*x))^m \[Star] Int[(a*c+(b*c+a*d)*x+b*d*x^2)^m,x] /;
FreeQ[{a,b,c,d},x] && LtQ[-1,m,0] && LeQ[3,Denominator[m],4] *)
...
IntLinearQ[a_,b_,c_,d_,m_,n_,x_Symbol] :=
  IGtQ[m,0] || IGtQ[n,0] || IntegersQ[4*m,4*n] || IntegersQ[2*m,6*n] || IntegersQ[6*m,2*n] || ILtQ[m+n,-1] || IntegerQ[m+n] && RationalQ[m] (* || IntegersQ[3*m,3*n] *)

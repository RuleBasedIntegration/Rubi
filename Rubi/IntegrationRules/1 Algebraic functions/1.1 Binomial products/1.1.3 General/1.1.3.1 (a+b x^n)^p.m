...
Int[1/((a_) + (b_.)*(x_)^2), x_Symbol] := (-(Rt[-a, 2]*Rt[-b, 2])^(-1))*ArcTan[Rt[-b, 2]*(x/Rt[-a, 2])] /; FreeQ[{a, b}, x] && PosQ[a/b] && (LtQ[a, 0] || LtQ[b, 0])
...
Int[1/((a_) + (b_.)*(x_)^2), x_Symbol] := (-(Rt[-a, 2]*Rt[b, 2])^(-1))*ArcTanh[Rt[b, 2]*(x/Rt[-a, 2])] /; FreeQ[{a, b}, x] && NegQ[a/b] && (LtQ[a, 0] || GtQ[b, 0])
...
Int[1/((a_) + (b_.)*(x_)^4), x_Symbol] := With[{r = Numerator[Rt[-(a/b), 2]], s = Denominator[Rt[-(a/b), 2]]}, (r/(2*a))*Int[1/(r - s*x^2), x] + (r/(2*a))*Int[1/(r + s*x^2), x]] /; FreeQ[{a, b}, x] &&  !GtQ[a/b, 0]
...
Int[1/((a_) + (b_.)*(x_)^(n_)), x_Symbol] := With[{r = Numerator[Rt[-(a/b), 2]], s = Denominator[Rt[-(a/b), 2]]}, (r/(2*a))*Int[1/(r - s*x^(n/2)), x] + (r/(2*a))*Int[1/(r + s*x^(n/2)), x]] /; FreeQ[{a, b}, x] && IGtQ[n/4, 1] &&  !GtQ[a/b, 0]
...
Int[1/Sqrt[(a_) + (b_.)*(x_)^4], x_Symbol] := (Sqrt[1 + b*(x^4/a)]/Sqrt[a + b*x^4])*Int[1/Sqrt[1 + b*(x^4/a)], x] /; FreeQ[{a, b}, x] && NegQ[b/a] &&  !GtQ[a, 0]
...
Int[((a_) + (b_.)*(x_)^(n_))^(p_), x_Symbol] := a^(p + 1/n)*Subst[Int[1/(1 - b*x^n)^(p + 1/n + 1), x], x, x/(a + b*x^n)^(1/n)] /; FreeQ[{a, b}, x] && IGtQ[n, 0] && LtQ[-1, p, 0] && NeQ[p, -2^(-1)] && IntegerQ[p + 1/n]
Int[((a_) + (b_.)*(x_)^(n_))^(p_), x_Symbol] := (a/(a + b*x^n))^(p + 1/n)*(a + b*x^n)^(p + 1/n)*Subst[Int[1/(1 - b*x^n)^(p + 1/n + 1), x], x, x/(a + b*x^n)^(1/n)] /; FreeQ[{a, b}, x] && IGtQ[n, 0] && LtQ[-1, p, 0] && NeQ[p, -2^(-1)] && LtQ[Denominator[p + 1/n], Denominator[p]]
...
Int[((a_) + (b_.)*(x_)^(n_))^(p_), x_Symbol] := a^p*x*Hypergeometric2F1[-p, 1/n, 1/n + 1, (-b)*(x^n/a)] /; FreeQ[{a, b, n, p}, x] &&  !IGtQ[p, 0] &&  !IntegerQ[1/n] &&  !ILtQ[Simplify[1/n + p], 0] && (IntegerQ[p] || GtQ[a, 0])
Int[((a_) + (b_.)*(x_)^(n_))^(p_), x_Symbol] := a^IntPart[p]*((a + b*x^n)^FracPart[p]/(1 + b*(x^n/a))^FracPart[p])*Int[(1 + b*(x^n/a))^p, x] /; FreeQ[{a, b, n, p}, x] &&  !IGtQ[p, 0] &&  !IntegerQ[1/n] &&  !ILtQ[Simplify[1/n + p], 0] &&  !(IntegerQ[p] || GtQ[a, 0])
...
Int[((a1_.) + (b1_.)*(x_)^(n_))^(p_)*((a2_.) + (b2_.)*(x_)^(n_))^(p_), x_Symbol] := (a1 + b1*x^n)^FracPart[p]*((a2 + b2*x^n)^FracPart[p]/(a1*a2 + b1*b2*x^(2*n))^FracPart[p])*Int[(a1*a2 + b1*b2*x^(2*n))^p, x] /; FreeQ[{a1, b1, a2, b2, n, p}, x] && EqQ[a2*b1 + a1*b2, 0] &&  !IntegerQ[p]

...
(* Int[(c_.*x_)^m_.*u_^p_*v_^p_,x_Symbol] :=
  ...
  c^n*(m-n+1)/(b*n*(p+1)) \[Star] Int[(c*x)^(m-n)*u^(p+1)*v^(p+1),x] /;
...
(* Int[x_^m_./(a_+b_.*x_^5),x_Symbol] :=
  ...
  (-1)^m*r^(m+1)/(5*a*s^m) \[Star] Int[1/(r+s*x),x] + 
  2*r^(m+1)/(5*a*s^m) \[Star] Int[(r*Cos[m*Pi/5]-s*Cos[(m+1)*Pi/5]*x)/(r^2-1/2*(1+Sqrt[5])*r*s*x+s^2*x^2),x] + 
  2*r^(m+1)/(5*a*s^m) \[Star] Int[(r*Cos[3*m*Pi/5]-s*Cos[3*(m+1)*Pi/5]*x)/(r^2-1/2*(1-Sqrt[5])*r*s*x+s^2*x^2),x]] /;
...
(* Int[x_^m_./(a_+b_.*x_^5),x_Symbol] :=
  With[{r=Numerator[Rt[-a/b,5]], s=Denominator[Rt[-a/b,5]]},
  (r^(m+1)/(5*a*s^m)) \[Star] Int[1/(r-s*x),x] + 
  2*(-1)^m*r^(m+1)/(5*a*s^m) \[Star] Int[(r*Cos[m*Pi/5]+s*Cos[(m+1)*Pi/5]*x)/(r^2+1/2*(1+Sqrt[5])*r*s*x+s^2*x^2),x] + 
  2*(-1)^m*r^(m+1)/(5*a*s^m) \[Star] Int[(r*Cos[3*m*Pi/5]+s*Cos[3*(m+1)*Pi/5]*x)/(r^2+1/2*(1-Sqrt[5])*r*s*x+s^2*x^2),x]] /;
...
(* ::Code:: *)
(* Int[x_/(a_+b_.*x_^3)^(1/3),x_Symbol] :=
  -a^2*(-b*x^3/a-b^2*x^6/a^2)^(1/3)/(3*2^(1/3)*b^2*x*(a+b*x^3)^(1/3)) \[Star] 
    Subst[Int[1/(1-a^2*x^2/b^2)^(1/3),x],x,-b/a-2*b^2*x^3/a^2] /;
FreeQ[{a,b},x] *)
...
IntBinomialQ[a_,b_,c_,n_,m_,p_,x_] :=
  IGtQ[p,0] || RationalQ[m] && IntegersQ[n,2*p] || IntegerQ[Simplify[(m+1)/n+p]] || 
  (EqQ[n,2] || EqQ[n,4]) && IntegersQ[2*m,4*p] || 
  EqQ[n,2] && IntegerQ[6*p] && (IntegerQ[m] || IntegerQ[m-p]) || 
  EqQ[n,3] && IntegerQ[m] && IntegerQ[(m+1)/n+p]

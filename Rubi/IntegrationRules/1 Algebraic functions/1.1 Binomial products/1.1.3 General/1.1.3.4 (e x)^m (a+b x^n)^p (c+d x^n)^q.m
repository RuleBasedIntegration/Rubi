...
(* ::Code:: *)
Int[x_^4/(Sqrt[a_+b_.*x_^4]*Sqrt[c_+d_.*x_^4]),x_Symbol] :=
...
(* ::Code:: *)
(* Int[x_^m_.*(a_+b_.*x_^n_)^p_*(c_+d_.*x_^n_)^q_,x_Symbol] :=
  -1/(m+1) \[Star] Subst[Int[(a+b*x^Simplify[-n/(m+1)])^p*(c+d*x^Simplify[-n/(m+1)])^q/x^2,x],x,x^(-(m+1))] /;
...
(* ::Code:: *)
(* Int[(e_.*x_)^m_.*(a_+b_.*x_^n_)^p_.*(c_+d_.*x_^n_)^q_.,x_Symbol] :=
  (e*x)^m/(n*x^(n-1)*(x^n)^(Simplify[(m+1)/n]-1)) \[Star] Subst[Int[x^(Simplify[(m+1)/n]-1)*(a+b*x)^p*(c+d*x)^q,x],x,x^n] /;
FreeQ[{a,b,c,d,e,m,n,p,q},x] && NeQ[b*c-a*d,0] *)

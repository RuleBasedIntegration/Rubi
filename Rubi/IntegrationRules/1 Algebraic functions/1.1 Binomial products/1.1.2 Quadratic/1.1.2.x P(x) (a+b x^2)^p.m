...
Int[(Pq_)*((a_) + (b_.)*(x_)^2)^(p_), x_Symbol] := With[{q = Expon[Pq, x], e = Coeff[Pq, x, Expon[Pq, x]]}, e*x^(q - 1)*((a + b*x^2)^(p + 1)/(b*(q + 2*p + 1))) + (1/(b*(q + 2*p + 1)))*Int[(a + b*x^2)^p*ExpandToSum[b*(q + 2*p + 1)*Pq - a*e*(q - 1)*x^(q - 2) - b*e*(q + 2*p + 1)*x^q, x], x]] /; FreeQ[{a, b, p}, x] && PolyQ[Pq, x] &&  !LeQ[p, -1]
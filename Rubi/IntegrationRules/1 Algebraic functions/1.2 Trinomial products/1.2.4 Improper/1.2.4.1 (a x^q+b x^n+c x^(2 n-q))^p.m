...
Int[((a_.)*(x_)^(q_.) + (b_.)*(x_)^(n_.) + (c_.)*(x_)^(r_.))^(p_), x_Symbol] := x*((a*x^q + b*x^n + c*x^(2*n - q))^p/(p*(2*n - q) + 1)) + (n - q)*(p/(p*(2*n - q) + 1))*Int[x^q*(2*a + b*x^(n - q))*(a*x^q + b*x^n + c*x^(2*n - q))^(p - 1), x] /; FreeQ[{a, b, c, n, q}, x] && EqQ[r, 2*n - q] && PosQ[n - q] &&  !IntegerQ[p] && NeQ[b^2 - 4*a*c, 0] && GtQ[p, 0] && NeQ[p*(2*n - q) + 1, 0]
Int[((a_.)*(x_)^(q_.) + (b_.)*(x_)^(n_.) + (c_.)*(x_)^(r_.))^(p_), x_Symbol] := (-x^(-q + 1))*(b^2 - 2*a*c + b*c*x^(n - q))*((a*x^q + b*x^n + c*x^(2*n - q))^(p + 1)/(a*(n - q)*(p + 1)*(b^2 - 4*a*c))) + (1/(a*(n - q)*(p + 1)*(b^2 - 4*a*c)))*Int[(((p*q + 1)*(b^2 - 2*a*c) + (n - q)*(p + 1)*(b^2 - 4*a*c) + b*c*(p*q + (n - q)*(2*p + 3) + 1)*x^(n - q))*(a*x^q + b*x^n + c*x^(2*n - q))^(p + 1))/x^q, x] /; FreeQ[{a, b, c, n, q}, x] && EqQ[r, 2*n - q] && PosQ[n - q] &&  !IntegerQ[p] && NeQ[b^2 - 4*a*c, 0] && LtQ[p, -1]
Int[((a_.)*(x_)^(q_.) + (b_.)*(x_)^(n_.) + (c_.)*(x_)^(r_.))^(p_), x_Symbol] := ((a*x^q + b*x^n + c*x^(2*n - q))^p/(x^(p*q)*(a + b*x^(n - q) + c*x^(2*(n - q)))^p))*Int[x^(p*q)*(a + b*x^(n - q) + c*x^(2*(n - q)))^p, x] /; FreeQ[{a, b, c, n, p, q}, x] && EqQ[r, 2*n - q] && PosQ[n - q] &&  !IntegerQ[p]

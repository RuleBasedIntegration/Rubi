...
(* ::Code:: *)
Int[x_*Px_*(d_+e_.*x_^2)^q_.*(a_+c_.*x_^4)^p_.,x_Symbol] :=
  1/2 \[Star] Subst[Int[ReplaceAll[Px,x->Sqrt[x]]*(d+e*x)^q*(a+c*x^2)^p,x],x,x^2] /;
FreeQ[{a,c,d,e,p,q},x] && PolyQ[Px,x^2]


(* ::Code:: *)
Int[Pr_*(f_.*x_)^m_.*(d_+e_.*x_^2)^q_.*(a_+b_.*x_^2+c_.*x_^4)^p_,x_Symbol] :=
...
(* ::Code:: *)
Int[Pr_*(f_.*x_)^m_.*(d_+e_.*x_^2)^q_.*(a_+c_.*x_^4)^p_,x_Symbol] :=
  Module[{r=Expon[Pr,x],k},
...
(* ::Code:: *)
Int[Px_*(f_.*x_)^m_.*(d_+e_.*x_^2)^q_.*(a_+b_.*x_^2+c_.*x_^4)^p_.,x_Symbol] :=
  Int[ExpandIntegrand[Px*(f*x)^m*(d+e*x^2)^q*(a+b*x^2+c*x^4)^p,x],x] /;
FreeQ[{a,b,c,d,e,f,m,q},x] && PolyQ[Px,x] && IntegerQ[p]


(* ::Code:: *)
Int[Px_*(f_.*x_)^m_.*(d_+e_.*x_^2)^q_.*(a_+c_.*x_^4)^p_.,x_Symbol] :=
...
(* ::Code:: *)
Int[Pr_*(d_+e_.*x_^2)^q_.*(a_+c_.*x_^4)^p_,x_Symbol] :=
  Module[{r=Expon[Pr,x],k},
...
FreeQ[{a,b,c,d,e},x] && PolyQ[Pq,x^2] && Expon[Pq,x^2]>0 && LtQ[p,-1] *)

...
Int[(Pq_)*((a_.) + (b_.)*(x_) + (c_.)*(x_)^2)^(p_), x_Symbol] := Int[x*PolynomialQuotient[Pq, x, x]*(a + b*x + c*x^2)^p, x] /; FreeQ[{a, b, c, p}, x] && PolyQ[Pq, x] && EqQ[Coeff[Pq, x, 0], 0] &&  !MatchQ[Pq, x^(m_.)*(u_.) /; IntegerQ[m]]
...
Int[(Pq_)*((a_.) + (b_.)*(x_) + (c_.)*(x_)^2)^(p_), x_Symbol] := With[{q = Expon[Pq, x], e = Coeff[Pq, x, Expon[Pq, x]]}, e*x^(q - 1)*((a + b*x + c*x^2)^(p + 1)/(c*(q + 2*p + 1))) + (1/(c*(q + 2*p + 1)))*Int[(a + b*x + c*x^2)^p*ExpandToSum[c*(q + 2*p + 1)*Pq - a*e*(q - 1)*x^(q - 2) - b*e*(q + p)*x^(q - 1) - c*e*(q + 2*p + 1)*x^q, x], x]] /; FreeQ[{a, b, c, p}, x] && PolyQ[Pq, x] && NeQ[b^2 - 4*a*c, 0] &&  !LeQ[p, -1]
...
Int[((a_) + (b_.)*(x_) + (c_.)*(x_)^2)^(p_), x_Symbol] := (b + 2*c*x)*((a + b*x + c*x^2)^p/(2*c*(2*p + 1))) /; FreeQ[{a, b, c, p}, x] && EqQ[b^2 - 4*a*c, 0] && NeQ[p, -2^(-1)]
...
Int[((a_.) + (b_.)*(x_) + (c_.)*(x_)^2)^(p_), x_Symbol] := Int[ExpandIntegrand[(a + b*x + c*x^2)^p, x], x] /; FreeQ[{a, b, c}, x] && NeQ[b^2 - 4*a*c, 0] && IGtQ[p, 0] && (EqQ[a, 0] ||  !PerfectSquareQ[b^2 - 4*a*c])
...
Int[1/((a_) + (b_.)*(x_) + (c_.)*(x_)^2), x_Symbol] := With[{q = 1 - 4*Simplify[a*(c/b^2)]}, (-2/b)*Subst[Int[1/(q - x^2), x], x, 1 + 2*c*(x/b)] /; RationalQ[q] && (EqQ[q^2, 1] ||  !RationalQ[b^2 - 4*a*c])] /; FreeQ[{a, b, c}, x] && NeQ[b^2 - 4*a*c, 0]
...
Int[((a_.) + (b_.)*(x_) + (c_.)*(x_)^2)^(p_), x_Symbol] := With[{q = Rt[b^2 - 4*a*c, 2]}, (-((a + b*x + c*x^2)^(p + 1)/(q*(p + 1)*((q - b - 2*c*x)/(2*q))^(p + 1))))*Hypergeometric2F1[-p, p + 1, p + 2, (b + q + 2*c*x)/(2*q)]] /; FreeQ[{a, b, c, p}, x] && NeQ[b^2 - 4*a*c, 0] &&  !IntegerQ[4*p]

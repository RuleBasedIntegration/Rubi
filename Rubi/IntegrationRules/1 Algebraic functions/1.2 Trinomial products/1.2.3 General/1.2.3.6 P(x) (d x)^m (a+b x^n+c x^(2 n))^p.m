...
(* ::Code:: *)
Int[x_^m_.*Px_*(a_+b_.*x_^n_+c_.*x_^n2_.)^p_.,x_Symbol] :=
...
(* ::Code:: *)
Int[(d_.*x_)^m_.*Px_*(a_+b_.*x_^n_.+c_.*x_^n2_.)^p_.,x_Symbol] :=
...
(* ::Code:: *)
(* Int[(d_.*x_)^m_.*Px_/(a_+b_.*x_^n_.+c_.*x_^n2_),x_Symbol] :=
  Int[ExpandIntegrand[(d*x)^m*Px/(a+b*x^n+c*x^(2*n)),x],x] /;
FreeQ[{a,b,c,d,m},x] && EqQ[n2,2*n] && PolyQ[Px,x^n] && NeQ[b^2-4*a*c,0] && IGtQ[n,0] && NiceSqrtQ[b^2-4*a*c] *)


(* ::Code:: *)
Int[Px_*(d_.*x_)^m_.*(a_+b_.*x_^n_.+c_.*x_^n2_)^p_,x_Symbol] :=
  With[{q=Expon[Px,x^n]},
...
(* ::Code:: *)
Int[u_^m_.*Px_*(a_+b_.*v_^n_+c_.*v_^n2_.)^p_.,x_Symbol] :=

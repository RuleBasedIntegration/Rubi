...
Int[((a_) + (b_.)*(x_)^(n_) + (c_.)*(x_)^(n2_.))^(p_), x_Symbol] := a^IntPart[p]*((a + b*x^n + c*x^(2*n))^FracPart[p]/((1 + 2*c*(x^n/(b + Rt[b^2 - 4*a*c, 2])))^FracPart[p]*(1 + 2*c*(x^n/(b - Rt[b^2 - 4*a*c, 2])))^FracPart[p]))*Int[(1 + 2*c*(x^n/(b + Sqrt[b^2 - 4*a*c])))^p*(1 + 2*c*(x^n/(b - Sqrt[b^2 - 4*a*c])))^p, x] /; FreeQ[{a, b, c, n, p}, x] && EqQ[n2, 2*n] && NeQ[b^2 - 4*a*c, 0] &&  !IntegerQ[p]
...
Int[((a_) + (b_.)*(x_)^(mn_) + (c_.)*(x_)^(n_.))^(p_), x_Symbol] := x^(n*FracPart[p])*((a + b/x^n + c*x^n)^FracPart[p]/(b + a*x^n + c*x^(2*n))^FracPart[p])*Int[(b + a*x^n + c*x^(2*n))^p/x^(n*p), x] /; FreeQ[{a, b, c, n, p}, x] && EqQ[mn, -n] &&  !IntegerQ[p] && PosQ[n]
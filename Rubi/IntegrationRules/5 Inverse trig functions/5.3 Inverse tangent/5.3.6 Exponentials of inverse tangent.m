...
(* Int[x_*(c_+d_.*x_^2)^p_*E^(n_.*ArcTan[a_.*x_]),x_Symbol] :=
  ...
  n*(2*p+3)/(a*c*(n^2+4*(p+1)^2)) \[Star] Int[(c+d*x^2)^(p+1)*E^(n*ArcTan[a*x]),x] /;
...
(* ::Code:: *)
(* Int[u_.*(c_+d_./x_^2)^p_*E^(n_*ArcTan[a_.*x_]),x_Symbol] :=
  x^(2*p)*(c+d/x^2)^p/((1-I*a*x)^p*(1+I*a*x)^p) \[Star] Int[u/x^(2*p)*(1-I*a*x)^p*(1+I*a*x)^p*E^(n*ArcTan[a*x]),x] /;

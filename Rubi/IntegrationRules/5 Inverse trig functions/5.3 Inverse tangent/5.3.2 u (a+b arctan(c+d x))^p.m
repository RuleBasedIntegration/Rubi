...
Int[((a_.) + (b_.)*ArcTan[(c_) + (d_.)*(x_)])^(p_), x_Symbol] := Unintegrable[(a + b*ArcTan[c + d*x])^p, x] /; FreeQ[{a, b, c, d, p}, x] &&  !IGtQ[p, 0]
Int[((a_.) + (b_.)*ArcCot[(c_) + (d_.)*(x_)])^(p_), x_Symbol] := Unintegrable[(a + b*ArcCot[c + d*x])^p, x] /; FreeQ[{a, b, c, d, p}, x] &&  !IGtQ[p, 0]
...
Int[((e_.) + (f_.)*(x_))^(m_.)*((a_.) + (b_.)*ArcTan[(c_) + (d_.)*(x_)])^(p_), x_Symbol] := Unintegrable[(e + f*x)^m*(a + b*ArcTan[c + d*x])^p, x] /; FreeQ[{a, b, c, d, e, f, m, p}, x] &&  !IGtQ[p, 0]
Int[((e_.) + (f_.)*(x_))^(m_.)*((a_.) + (b_.)*ArcCot[(c_) + (d_.)*(x_)])^(p_), x_Symbol] := Unintegrable[(e + f*x)^m*(a + b*ArcCot[c + d*x])^p, x] /; FreeQ[{a, b, c, d, e, f, m, p}, x] &&  !IGtQ[p, 0]
...
Int[ArcTan[(a_) + (b_.)*(x_)]/((c_) + (d_.)*(x_)^(n_)), x_Symbol] := Unintegrable[ArcTan[a + b*x]/(c + d*x^n), x] /; FreeQ[{a, b, c, d, n}, x] &&  !RationalQ[n]
Int[ArcCot[(a_) + (b_.)*(x_)]/((c_) + (d_.)*(x_)^(n_)), x_Symbol] := Unintegrable[ArcCot[a + b*x]/(c + d*x^n), x] /; FreeQ[{a, b, c, d, n}, x] &&  !RationalQ[n]

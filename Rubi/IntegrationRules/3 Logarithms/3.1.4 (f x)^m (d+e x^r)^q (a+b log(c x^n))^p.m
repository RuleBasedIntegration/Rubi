...
(* Int[x_^m_.*(a_.+b_.*Log[c_.*x_^n_.])^p_./(d_+e_.*x_^r_.),x_Symbol] :=
  1/e \[Star] Int[x^(m-r)*(a+b*Log[c*x^n])^p,x] - 
  d/e \[Star] Int[(x^(m-r)*(a+b*Log[c*x^n])^p)/(d+e*x^r),x] /;
...
(* Int[(a_.+b_.*Log[c_.*x_^n_.])^p_./(x_*(d_+e_.*x_^r_.)),x_Symbol] :=
  ...
  b*n*p/d \[Star] Int[Log[x]*(a+b*Log[c*x^n])^(p-1)/x,x] + 
  b*n*p/(d*r) \[Star] Int[Log[1+(e*x^r)/d]*(a+b*Log[c*x^n])^(p-1)/x,x] /;

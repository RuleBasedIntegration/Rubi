...
Int[((a_) + (b_.)*(x_)^(n_))^(p_)*Sinh[(c_.) + (d_.)*(x_)], x_Symbol] := x^(-n + 1)*(a + b*x^n)^(p + 1)*(Sinh[c + d*x]/(b*n*(p + 1))) - ((-n + 1)/(b*n*(p + 1)))*Int[((a + b*x^n)^(p + 1)*Sinh[c + d*x])/x^n, x] - (d/(b*n*(p + 1)))*Int[x^(-n + 1)*(a + b*x^n)^(p + 1)*Cosh[c + d*x], x] /; FreeQ[{a, b, c, d}, x] && IntegerQ[p] && IGtQ[n, 0] && LtQ[p, -1] && GtQ[n, 2]
Int[((a_) + (b_.)*(x_)^(n_))^(p_)*Cosh[(c_.) + (d_.)*(x_)], x_Symbol] := x^(-n + 1)*(a + b*x^n)^(p + 1)*(Cosh[c + d*x]/(b*n*(p + 1))) - ((-n + 1)/(b*n*(p + 1)))*Int[((a + b*x^n)^(p + 1)*Cosh[c + d*x])/x^n, x] - (d/(b*n*(p + 1)))*Int[x^(-n + 1)*(a + b*x^n)^(p + 1)*Sinh[c + d*x], x] /; FreeQ[{a, b, c, d}, x] && IntegerQ[p] && IGtQ[n, 0] && LtQ[p, -1] && GtQ[n, 2]

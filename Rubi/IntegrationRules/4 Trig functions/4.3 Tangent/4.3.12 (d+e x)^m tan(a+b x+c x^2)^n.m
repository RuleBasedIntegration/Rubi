...
(* Int[x_^m_*Tan[a_.+b_.*x_+c_.*x_^2],x_Symbol] :=
  ...
  b/(2*c) \[Star] Int[x^(m-1)*Tan[a+b*x+c*x^2],x] + 
  (m-1)/(2*c) \[Star] Int[x^(m-2)*Log[Cos[a+b*x+c*x^2]],x] /;
...
(* Int[x_^m_*Cot[a_.+b_.*x_+c_.*x_^2],x_Symbol] :=
  ...
  b/(2*c) \[Star] Int[x^(m-1)*Cot[a+b*x+c*x^2],x] -
  (m-1)/(2*c) \[Star] Int[x^(m-2)*Log[Sin[a+b*x+c*x^2]],x] /;
FreeQ[{a,b,c},x] && GtQ[m,1]*)
